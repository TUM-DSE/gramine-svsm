...
// NOTE: This file is adopted from the gramine-tdx and modified
// The orignal version: https://github.com/gramineproject/gramine-tdx/blob/b6e97234ec8afce4a5fb6ce94aeea6869a3bb82e/pal/src/host/vm-common/pal_common_tf.c
// Main changes:
// - tdx -> svsm
// - use guest request to read file insted of using virtiofs
...
int load_trusted_or_allowed_file(struct trusted_file* tf, struct pal_handle* file, bool create,
    ...
    *out_chunk_hashes = NULL;
    ...
    svsm_chunk_hash_t* chunk_hashes = NULL;
    ...
    chunk_hashes = malloc(sizeof(svsm_chunk_hash_t) * UDIV_ROUND_UP(tf->size, TRUSTED_CHUNK_SIZE));
    if (!chunk_hashes) {
        ret = -PAL_ERROR_NOMEM;
        goto fail;
    }
    ...
    svsm_chunk_hash_t* chunk_hashes_item = chunk_hashes;
    uint64_t offset = 0;
    ...
    for (; offset < tf->size; offset += TRUSTED_CHUNK_SIZE, chunk_hashes_item++) {
        ...
        svsm_chunk_hash_t chunk_hash[2]; /* each chunk_hash is 128 bits in size */
    ...
    if (memcmp(&file_hash, &tf->file_hash, sizeof(file_hash))) {
        log_warning("Hash of trusted file '%s' does not match with the reference hash in manifest",
                    file->file.realpath);
        char buf[2 * sizeof(file_hash.bytes) + 1];
        log_warning("Expected hash: %s", bytes2hex(tf->file_hash.bytes, sizeof(tf->file_hash.bytes), buf, sizeof(buf)));
        log_warning("Actual hash: %s", bytes2hex(file_hash.bytes, sizeof(file_hash.bytes), buf, sizeof(buf)));
...
int copy_and_verify_trusted_file(struct pal_handle* file, uint8_t* buf, int64_t aligned_offset,
    ...
    svsm_chunk_hash_t* chunk_hashes = (svsm_chunk_hash_t*)_chunk_hashes;
    int ret = 0;
    ...
    svsm_chunk_hash_t* chunk_hashes_item = chunk_hashes + aligned_offset / TRUSTED_CHUNK_SIZE;
    ...
    for (; chunk_offset < aligned_end; chunk_offset += TRUSTED_CHUNK_SIZE, chunk_hashes_item++) {
        ...
        svsm_chunk_hash_t chunk_hash[2]; /* each chunk_hash is 128 bits in size but we need 256 */
        ...
        if (memcmp(chunk_hashes_item, &chunk_hash[0], sizeof(*chunk_hashes_item))) {
            ...
            char buf[2 * sizeof(*chunk_hashes_item) + 1];
            log_error("Expected hash: %s", bytes2hex(chunk_hashes_item->bytes, sizeof(*chunk_hashes_item), buf, sizeof(buf)));
            log_error("Actual hash: %s", bytes2hex(chunk_hash[0].bytes, sizeof(chunk_hash[0]), buf, sizeof(buf)));
            ret = -PAL_ERROR_DENIED;
            goto failed;
        }
    }
...
static int register_file(const char* uri, const char* hash_str, bool check_duplicates) {
    if (hash_str && strlen(hash_str) != sizeof(svsm_file_hash_t) * 2) {
    ...
    if (hash_str) {
        assert(strlen(hash_str) == sizeof(svsm_file_hash_t) * 2);
...
int init_trusted_files(void) {
    ...
    toml_table_t* manifest_svsm = toml_table_in(g_pal_public_state.manifest_root, "svsm");
    if (!manifest_svsm) {
        /* hack to re-use `sgx` key if `svsm` not found */
        manifest_svsm = toml_table_in(g_pal_public_state.manifest_root, "sgx");
    }
    if (!manifest_svsm)
        return 0;

    toml_array_t* toml_trusted_files = toml_array_in(manifest_svsm, "trusted_files");
    ...
    for (ssize_t i = 0; i < toml_trusted_files_cnt; i++) {
        /* read `svsm.trusted_file = {uri = "file:foo", sha256 = "deadbeef"}` entry from manifest */
...
int init_allowed_files(void) {
    ...
    toml_array_t* toml_allowed_files = toml_array_in(manifest_svsm, "allowed_files");
...
int init_file_check_policy(void) {
    ...
    ret = toml_string_in(g_pal_public_state.manifest_root, "svsm.file_check_policy",
                         &file_check_policy_str);
    if (ret < 0) {
        log_error("Cannot parse 'svsm.file_check_policy'");
        return -PAL_ERROR_INVAL;
    }
    ...
    if (!strcmp(file_check_policy_str, "strict")) {
        ...
        log_error("Unknown value for 'svsm.file_check_policy' "
